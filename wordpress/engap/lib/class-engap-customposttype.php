 ...
 * A sensible base for custom post type APIs
 */
abstract class ENGAP_CustomPostType extends ENGAP_Posts {
	/**
	 * Base route name
	 *
	...
	public function __construct(ENGAP_ResponseHandler $server) {
		if ( empty( $this->base ) ) {
			_doing_it_wrong( 'ENGAP_CustomPostType::__construct', __( 'The route base must be overridden' ), 'WPAPI-0.6' );
			return;
		}
		if ( empty( $this->type ) ) {
			_doing_it_wrong( 'ENGAP_CustomPostType::__construct', __( 'The post type must be overridden' ), 'WPAPI-0.6' );
			return;
		}
	...
	public function register_revision_routes( $routes ) {
		$routes[ $this->base . '/(?P<id>\d+)/revisions' ] = array(
			array( array( $this, 'get_revisions' ), ENGAP_Server::READABLE ),
		);
		return $routes;
	}
	...
	public function register_comment_routes( $routes ) {
		$routes[ $this->base . '/(?P<id>\d+)/comments'] = array(
			array( array( $this, 'get_comments' ), ENGAP_Server::READABLE ),
		);
		$routes[ $this->base . '/(?P<id>\d+)/comments/(?P<comment>\d+)' ] = array(
			array( array( $this, 'get_comment' ), ENGAP_Server::READABLE ),
			array( array( $this, 'delete_comment' ), ENGAP_Server::DELETABLE ),
		);
		return $routes;
	}
