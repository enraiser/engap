 ...
 * WordPress ENGAP 
 *
 * Contains the ENGAP_Server class.
 *
 * @package WordPress
 */
...
/**
 * WordPress ENGAP  server handler
 *
 * @package WordPress
 */
class ENGAP_Server implements ENGAP_ResponseHandler {
	const METHOD_GET    = 1;
	const METHOD_POST   = 2;
	const METHOD_PUT    = 4;
	...
	protected function error_to_response( $error ) {
		...
		$response = new ENGAP_Response( $data, $status );
	 ...
	 *
	 * Note: This should only be used in {@see ENGAP_Server::serve_request()},
	...
	public function serve_request( $path = null ) {
		...
		// Proper filter for turning off the ENGAP . It is on by default.
		$enabled = apply_filters( 'json_enabled', true );

		$jsonp_enabled = apply_filters( 'json_jsonp_enabled', true );

		if ( ! $enabled ) {
			echo $this->json_error( 'json_disabled', __( 'The ENGAP  is disabled on this site.' ), 404 );
			return false;
		}
		...
		if ( ! is_wp_error( $result ) ) {
			 ...
			 * @param ENGAP_ResponseHandler $this ResponseHandler instance (usually ENGAP_Server)
			 */
		...
		if ( $result instanceof ENGAP_ResponseInterface ) {
	...
	public function prepare_response( $data ) {
		if ( ! defined( 'ENGAP_SERIALIZE_COMPATIBLE' ) || ENGAP_SERIALIZE_COMPATIBLE === false ) {
			return $data;
		}
